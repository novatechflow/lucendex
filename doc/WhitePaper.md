...
4. Why Not Native Pathfinding?  
5. LucenDEX System Design  
   * XRPL Native Layer  
...
**Lucendex** is conceived as a **neutral execution layer on XRPL** whose architecture is tuned to withstand extreme stress: no custody, deterministic execution, robust quoting, smart routing, and on-chain transparency. Our mission: to provide a trading platform with **trust by design**, avoiding many of the failure modes that imperiled CEXs during the recent crash.
...
**Short answer:** native `path_find` is a great primitive, but it does not provide the infrastructure guarantees required by partners.

| Capability | Native `path_find` | LucenDEX |
|-----------|-------------------|---------|
| Circuit breakers | ❌ | ✅ Price anomaly detection + auto-pause |
| Quote binding | ❌ | ✅ blake2b-256 QuoteHash (tamper-evident) |
...
LucenDEX is built as **infrastructure**, not a UI wrapper.
...
2. **Phase 2**: Margin trading (overcollateralized) with guardrails + LendingProtocol opportunities (XRPL 3.1.0).  
...
XRPL 3.1.0 introduces LendingProtocol (XLS-66), which opens new paths for institutional margin workflows and liquidity funding.  
LucenDEX remains fully non-custodial and treats LendingProtocol as **Phase 2** scope, not V1.

**Planned alignment:**
* Evaluate LendingProtocol primitives for atomic leveraged swaps and inventory funding.
* Keep QuoteHash binding and circuit breakers as mandatory guardrails.
* Maintain partner-only exposure until risk controls are proven in production.
